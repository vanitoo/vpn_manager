...
async def auth_middleware(request: web.Request, handler):
    if request.path == '/health':
        # Health is intentionally still protected when a token is configured.
...
async def health(request: web.Request) -> web.Response:
    ...
    return web.json_response({
        'ok': bool(DRY_RUN or APPLY_COMMAND),
        'mode': 'dry-run' if DRY_RUN else 'command',
        'configured': bool(DRY_RUN or APPLY_COMMAND),
        'active': active,
    })
...
async def put_user(request: web.Request) -> web.Response:
    ...
    async with LOCK:
        ...
        if not ok:
            ...
            # Best effort restore backend from the previous manifest.
            await _apply_backend()
            raise web.HTTPServiceUnavailable(text=detail)
    return web.json_response({'ok': True, 'telegram_id': numeric_id, 'enabled': enabled})
